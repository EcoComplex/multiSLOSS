...
mdl <- sloss_sim %>% filter(deforestation_type == "regular", birds_behavior=="BirthSelection")
names(mdl)
#
# summarize data 
#
...
mdl <- sloss_sim %>% filter(deforestation_type == "regular")
names(mdl)
#
# summarize data 
#
...
mdl1 %>%  filter(birth_rate_birds == 2) %>% 
  ggplot(aes(x = habitat_patch_size, y = calc_number_of_species_mean, color = factor(birds_dispersal_distance))) + 
  geom_line(aes(group = interaction(birds_dispersal_distance, birds_behavior)),  size = 1.2) +
...
## Not fragmented communities Lambda = 1.7 - 4 - No Selection communities 

* Initial conditions: migration from the regional community and after the area is filled of individuals, then 60% of the area degraded with regular patches (t= 200 for lambda=4 ) 

```{r NoSelection_SLOSS_lambda14_notfrag,echo=FALSE,message=FALSE,warning=FALSE,eval=FALSE}

# pf = probability of fragmentation, hps = habitat patch size , dd=dispersal distance 
# hps = habitat patch size
# dd = dispersal distance 
# brb = bird rate birds 
nl@experiment <- experiment(expname="NoSelection_pf00_nofrag_dd1-3_brb1-4_regular",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="false",
                            idsetup="setup",
                            idgo="go",
                            runtime=0,
                            metrics=c("birds-at-deforestation", "calc-number-of-species","calc-shannon-diversity","count birds"),
                            variables = list("birds-dispersal-distance" = list(values=c(1.1, 1.2, 1.5, 2, 3)),
                                              "birth-rate-birds" = list(values=c(1.7, 2, 3, 4))
                                             ),
                            constants = list("world-width" = 199,
...
## Read Simulations NoFragmentation No Selection  regular Patterns


```{r readSimulationsNoFragNSelectionRegular, echo=FALSE,message=FALSE,warning=FALSE,eval=FALSE}
# Read previous simulations
#
if( file.exists("Simulations/sloss_sim_ff.rds")) {
...
# Convert the list of mean_free_path to degraded_mfp and non_degraded_mfp
#
mdl <- read_sloss_simul(outpath,"NoSelection_pf00_nofrag_dd1-3_brb1-4_regular_ff.csv") %>% 
  mutate(habitat_patch_size = world_width, max_patch = (world_width+1) * (world_height+1) ) 
...
## Not fragmented communities Lambda = 1.7 - 4 - Hierarchical communities 

```{r Hierarchical_SLOSS_lambda14_notfrag,echo=FALSE,message=FALSE,warning=FALSE,eval=FALSE}

# pf = probability of fragmentation, hps = habitat patch size , dd=dispersal distance 
# hps = habitat patch size
# dd = dispersal distance 
# brb = bird rate birds 
nl@experiment <- experiment(expname="Hierarchical_pf00_nofrag_dd1-3_brb1-4_regular",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="false",
                            idsetup="setup",
...
## Read Simulations Not Fragmented  Hierarchical = habitat_patch_size == 200 


```{r readSimulationsNoFragHierarchical, echo=FALSE,message=FALSE,warning=FALSE,eval=FALSE}
# Read previous simulations
#
if( file.exists("Simulations/sloss_sim_ff.rds")) {
...
# Convert the list of mean_free_path to degraded_mfp and non_degraded_mfp
#
mdl <- read_sloss_simul(outpath,"Hierarchical_pf00_nofrag_dd1-3_brb1-4_regular_ff.csv") %>% 
  mutate(habitat_patch_size = world_width, max_patch = (world_width+1) * (world_height+1) ) 
...
#mdl <- sloss_sim %>% filter( Deforestation_at_time==0)

```


## Not fragmented communities Lambda = 1.7 - 4 - Selection communities 

```{r BirthSelection_SLOSS_lambda14_notfrag,echo=FALSE,message=FALSE,warning=FALSE,eval=TRUE}

# pf = probability of fragmentation, hps = habitat patch size , dd=dispersal distance 
# hps = habitat patch size
# dd = dispersal distance 
# brb = bird rate birds 
nl@experiment <- experiment(expname="BirthSelection_pf00_nofrag_dd1-3_brb1-4_regular",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="false",
                            idsetup="setup",
...
```{r readSimulationsNoFragBirthSelection, echo=FALSE,message=FALSE,warning=FALSE,eval=FALSE}
# Read previous simulations
#
if( file.exists("Simulations/sloss_sim_ff.rds")) {
...
# Convert the list of mean_free_path to degraded_mfp and non_degraded_mfp
#
mdl <- read_sloss_simul(outpath,"BirthSelection_pf00_nofrag_dd1-3_brb1-4_regular_ff.csv") %>% 
  mutate(habitat_patch_size = world_width, max_patch = (world_width+1) * (world_height+1) ) 
